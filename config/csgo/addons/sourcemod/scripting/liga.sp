...
public void OnPluginStart() {
  ...
  if(gameEngine == Engine_CSS) {
    ...
    RegAdminCmd("mp_swapteams", Command_CSS_SwapTeams, ADMFLAG_SLAY, "Swaps the teams.");
    RegConsoleCmd("motd", Command_CSS_MOTD, "Shows the info MOTD.");
    RegConsoleCmd("score", Command_CSS_Score, "Shows the score.");
    ...
    // disable the team and class select menus
    HookUserMessage(GetUserMessageId("VGUIMenu"), Hook_CSS_Force_Team, true);
  }
 ...
 */
public Action Command_CSS_MOTD(int id, int args) {
  if(!IsFakeClient(id)) {
    ShowMOTDPanel(id, hostname, "cfg/motd.txt", 2);
  }
 ...
 * @param args The command arguments.
 */
public Action Command_CSS_Score(int id, int args) {
 ...
 * @param args The command arguments.
 */
public Action Command_CSS_SwapTeams(int id, int args) {
 ...
 */
public Action Event_CSS_TidyChat(Event event, const char[] name, bool dontBroadcast) {
 ...
 * @param players     Array containing player indices.
 * @param playersNum  Number of players in the array.
 * @param reliable    If the message is reliable.
 * @param init        If an init message.
 */
public Action Hook_CSS_Force_Team(UserMsg msgId, Protobuf msg, const int[] players, int playersNum, bool reliable, bool init) {
  BfReadString(msg, buffer, BUFFER_SIZE_MAX);

  if(
    !StrEqual(buffer, MENU_TEAM_SELECT)
    && !StrEqual(buffer, MENU_TEAM_SELECT_CT)
    && !StrEqual(buffer, MENU_TEAM_SELECT_T)
  ) {
    LogToGame("menu detected - %s", buffer);
    return Plugin_Continue;
  }
